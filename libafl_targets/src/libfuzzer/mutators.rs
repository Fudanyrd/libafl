    ...
    SM::Mutations: MutatorsTuple<BytesInput, S>,
{
    ...
    fn scheduled_mutate(
        ...
        if new_len > len_max {
            return Err(Error::illegal_state(format!("LLVMFuzzerCustomMutator returned more bytes than allowed. Expected up to {len_max} but got {new_len}")));
        }
    ...
    SM::Mutations: MutatorsTuple<BytesInput, S>,
{
    ...
    fn scheduled_mutate(
        ...
        if new_len > len_max {
            return Err(Error::illegal_state(format!("LLVMFuzzerCustomCrossOver returned more bytes than allowed. Expected up to {len_max} but got {new_len}")));
        }
