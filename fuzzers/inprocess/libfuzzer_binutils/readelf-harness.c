...
// A fake, read-only, in-memory file struct.
...
struct fake_file *fake_fopen(uint8_t *data, size_t len) {
    struct fake_file *ff = calloc(1, sizeof(struct fake_file));
    ff->file_data = data;
    ff->len = len;
    ff->offset = 0;
    return ff;
...
void fake_fclose(FakeFile *ff) {
    free(ff);
}

int fake_fseek(FakeFile *stream, long offset, int whence)
{
    switch (whence) {
    case (SEEK_SET): {
    ...
    case (SEEK_END): {
        stream->offset = stream->len + offset;
        break;
    }
    default: {
        return -1;
...
int fake_fscanf(FakeFile *stream, const char *format, ...) {
    const char *p = stream->file_data + stream->offset;
    va_list ap; 
	va_start(ap, format);
    return vsscanf(p, format, ap);
}

// Fake rewind.
#define fake_rewind(stream) do { fake_fseek((stream), 0L, SEEK_SET); } while (0)

/**
 * @param size length of each item
 * @param nmemb number of items to read
 */
size_t fake_fread(void *ptr, size_t size, size_t nmemb, FakeFile *stream)
{
    assert(stream->offset >= 0);

	// calculate number of bytes to read.
    size_t nread = size * nmemb;
    if (stream->offset + nread > stream->len) {
        nread = stream->len - stream->offset;
    }
    ...
    // copy to ptr.
    memcpy(ptr, stream->file_data + stream->offset, nread);
    // update file offset.
    stream->offset += nread;
...
long fake_ftell(FakeFile *stream)
{
	assert(stream->offset >= 0);
	  ...
	  const char *   reason)
{
  ...
  if (fake_fseek (filedata->handle, archive_file_offset + offset, SEEK_SET))
    {
      if (reason)
  ...
  if (fake_fread (mvar, (size_t) size, (size_t) nmemb, filedata->handle) != nmemb)
    {
      if (reason)
...
process_program_headers (Filedata * filedata)
{
  ...
  for (i = 0, segment = filedata->program_headers;
	...
	case PT_INTERP:
	  if (fake_fseek (filedata->handle, archive_file_offset + (long) segment->p_offset,
		     SEEK_SET))
	    error (_("Unable to find program interpreter name\n"));
	  else
	    {
			fprintf(stderr, "unable to get interpreter name because fake_fscanf is not implemented. \n");
	    }
	  break;
	}
    }
...
get_dynamic_data (Filedata * filedata, bfd_size_type number, unsigned int ent_size)
{
  ...
  if (fake_fread (e_data, ent_size, (size_t) number, filedata->handle) != number)
    {
...
process_symbol_table (Filedata * filedata)
{
  ...
  if (dynamic_info[DT_HASH]
		 ...
		 SEEK_SET))
	{
      ...
      if (fake_fread (nb, hash_ent_size, 1, filedata->handle) != 1)
	{
	  error (_("Failed to read in number of buckets\n"));
	  goto no_hash;
	}

      if (fake_fread (nc, hash_ent_size, 1, filedata->handle) != 1)
	{
  ...
  if (dynamic_info_DT_GNU_HASH
		 ...
		 SEEK_SET))
	{
      ...
      if (fake_fread (nb, 16, 1, filedata->handle) != 1)
	{
	  error (_("Failed to read in number of buckets\n"));
	  goto no_gnu_hash;
	}
		 ...
		 SEEK_SET))
	{
      ...
      do
	{
	  if (fake_fread (nb, 4, 1, filedata->handle) != 1)
	    {
...
get_file_header (Filedata * filedata)
{
  ...
  if (fake_fread (filedata->file_header.e_ident, EI_NIDENT, 1, filedata->handle) != 1)
    return FALSE;
  ...
  if (is_32bit_elf)
    {
      ...
      if (fake_fread (ehdr32.e_type, sizeof (ehdr32) - EI_NIDENT, 1, filedata->handle) != 1)
	return FALSE;
...
64 bit data type and so it cannot read 64 bit ELF files.\n"));
      ...
      if (fake_fread (ehdr64.e_type, sizeof (ehdr64) - EI_NIDENT, 1, filedata->handle) != 1)
	return FALSE;
...
open_file (const char * pathname)
{
  ...
  // use our in-memory file.
  filedata->handle = fake_fopen (input_data, input_size);
  if (filedata->handle == NULL)
    goto fail;

  filedata->file_size = (bfd_size_type) input_size;
...
process_archive (Filedata * filedata, bfd_boolean is_thin_archive) {
	// not implemented because it requires reading another file
	// on disk other than the one already loaded into memory.
	fprintf(stderr, "process_archive not implemented\n");
	return TRUE;
}
...
process_file (char * file_name)
{
  ...
  if (fake_fread (armag, SARMAG, 1, filedata->handle) != 1)
    {
...
// original entry point of readelf.
int
readelf_main (int argc, char ** argv)
{
  int err;
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
	char *argv[] = {
		...
		"-x", /* --hex-dump */
		"-p", /* --string-dump */
		"-D", /* Use the dynamic section info */
		"-z", /* decompress */
		"foo", /* File name is not used. */
		"bar", /* File name is not used. */
		NULL,
	};
